...
DATA_VERSION = "031626"
APP_NAME = "podcast"
DESC = "RSS-first podcast fetcher/downloader with queues & playlists"


def xdg_path(env_key: str, default_sub: str) -> str:
    """
    Return a base path using XDG environment variable if set,
    otherwise fallback to ~/default_sub. Always appends APP_NAME.
    """
    base = os.environ.get(env_key)
    if base:
        return os.path.expanduser(base)
    home = os.path.expanduser("~")
    return os.path.join(home, default_sub)


# Config & data directories
CONFIG_DIR = xdg_path("XDG_CONFIG_HOME", "apps/.app-data/.config-files/podcast")
DATA_DIR = xdg_path("XDG_DATA_HOME", "apps/pod/.data/rnddata")

DB_PATH = os.path.join(DATA_DIR, f"{APP_NAME}-dv{DATA_VERSION}.db")
PODFILE_PATH = os.path.join(CONFIG_DIR, "PODFILE.ini")

# Default audio downloads go here (can be overridden by "download_dir" setting)
DOWNLOADS_DIR = os.path.join(
    xdg_path("XDG_MUSIC_DIR", "Music/pods")
)

# Default yt-dlp downloads dir (can be overridden by `podcast set --download-yt-dir`)
DEFAULT_YT_DIR = xdg_path("XDG_VIDEOS_DIR", "Videos/yt")
...
def ensure_minimal_dirs_only():
    # Ensure config & data dirs exist before reading/writing PodFile / DB
...
def connect_db():
    ...
    -- Metadata for conditional HTTP (ETag/Last-Modified)
    ...
    -- Helpful indices
    CREATE INDEX IF NOT EXISTS idx_episodes_feed_pub ON episodes(feed, pub_date);
    CREATE UNIQUE INDEX IF NOT EXISTS ux_episodes_feed_guid ON episodes(feed, guid);
    CREATE INDEX IF NOT EXISTS idx_downloads_ep_status ON downloads(episode_id, status);
    ...
    try:
        cur.execute(
            ...
            USING fts5(title, description, content='episodes', content_rowid='id');
            """
        )
        cur.executescript(
            """
        ...
        # FTS5 not available on this SQLite build; ignore.
        pass
...
def slugify(text: str, maxlen: int = 120) -> str:
    ...
    text = text[:maxlen] if text else "episode"
...
def parse_date(s: Optional[str]) -> Optional[str]:
    ...
    # As a last resort, return the original (so we don't lose data)
    return s.strip()


def ago_str(iso: Optional[str]) -> str:
    if not iso:
        return ""
    try:
        d = _parse_dt(iso) or dt.datetime.fromisoformat(iso.replace("Z", "+00:00"))
...
def get_changelog_text():
    try:
        return pkg_resources.files("podcast").joinpath("CHANGELOG.md").read_text(encoding="utf-8")
    except Exception:
...
def http_get(
    ...
    try:
        ...
        # e.code present; for 304 Not Modified we return with no body
        if e.code == 304:
            return 304, dict(e.headers.items()), b""
        raise


def http_stream_to_file(
    url: str, out_path: str, max_retries: int = 3
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            with urllib.request.urlopen(req, timeout=HTTP_TIMEOUT) as resp:
                ...
                if ctype and not ("audio" in ctype or "octet-stream" in ctype):
                    eprint(f"  Warning: unexpected content-type: {ctype}")
...
def parse_feed_rss(xml_bytes: bytes) -> List[Dict[str, Optional[str]]]:
    ...
    if not tag.endswith("rss") and "rdf" not in tag:
        raise SystemExit(
            f"Unsupported feed format (root tag '{root.tag}'). RSS is required."
        )
    ...
    items = [
        i for i in items if i.get("title") or i.get("enclosure_url") or i.get("guid")
    ]
    if not items:
...
# ---------- Feed conditional fetching ----------


def _get_conditional_headers(
    conn: sqlite3.Connection, feed_short: str
) -> Dict[str, str]:
    row = conn.execute(
        "SELECT etag, last_modified FROM feed_meta WHERE feed = ?", (feed_short,)
    ).fetchone()
...
def _update_feed_meta(conn: sqlite3.Connection, feed_short: str, hdrs: Dict[str, str]):
    etag = hdrs.get("ETag") or hdrs.get("Etag")
    lastm = hdrs.get("Last-Modified") or hdrs.get("Last-modified")
    conn.execute(
        "INSERT INTO feed_meta(feed, etag, last_modified, updated_at) VALUES (?, ?, ?, ?) "
        "ON CONFLICT(feed) DO UPDATE SET etag=excluded.etag, last_modified=excluded.last_modified, updated_at=excluded.updated_at",
        (feed_short, etag, lastm, now_iso()),
    )
    conn.commit()
...
def cmd_init(args):
    ...
    print(
        ...
        f"  Database:  {DB_PATH}\n"
        f"  Downloads base: {get_base_downloads_dir()} (feeds stored in subfolders)\n"
...
def cmd_dirs(args):
    ...
    base = os.path.abspath(get_base_downloads_dir())
    print(f"Base audio dir: {base}\n")
    ...
    print("Feed directories:")

    for feed in sorted(feeds.keys()):
        path = os.path.abspath(get_feed_download_dir(feed))

        if feed in overrides:
            print(f"  {feed:<20} -> {path}  (override)")
        else:
            print(f"  {feed:<20} -> {path}")
...
def cmd_rename_feed(args):
    ...
    with connect_db() as conn:
        ...
        conn.execute("UPDATE feed_meta SET feed = ? WHERE feed = ?", (new, old))
        conn.commit()
    ...
    # Filesystem move logic if no override
...
def _fetch_feed_with_conditionals(
    conn: sqlite3.Connection, short: str, url: str
) -> Optional[bytes]:
    headers = _get_conditional_headers(conn, short)
    try:
        status, hdrs, body = http_get(url, headers=headers)
        if status == 304:
            print(f"  Not modified (ETag/Last-Modified).")
            return None
        _update_feed_meta(conn, short, hdrs)
        return body
    except Exception as e:
...
def cmd_refresh(args):
    ...
    with connect_db() as conn:
        for short in targets:
            ...
            xml = _fetch_feed_with_conditionals(conn, short, url)
            if xml is None:
                continue  # unchanged or fetch error already printed
            try:
            ...
            conn.commit()
            print(f"  Added {added} new episode record(s)")
...
def cmd_info(args):
    ...
    ago = f" ({ago_str(last_pub_s)})" if last_pub_s and last_pub_s != "N/A" else ""
    print(f"Feed:         {short}")
    print(f"URL:          {url}")
    print(f"Format:       RSS (enforced)")
    print(f"Download dir: {get_feed_download_dir(short)}")
    print(f"Episodes:     {c1}")
    print(f"Downloaded:   {c2}")
    print(f"Last publish: {last_pub_s}{ago}")
    if summary:
        print("\nSummary:")
...
def _episodes_for_download(
    ...
    base = "SELECT e.* FROM episodes e LEFT JOIN downloads d ON d.episode_id = e.id AND d.status = 'completed' WHERE e.feed = ? AND d.id IS NULL"
    args = [feed]
    if ids:
    ...
    if since:
        ...
        args.append(since)
    # Newest first; non-null pub_date first, then id desc
    base += " ORDER BY (e.pub_date IS NULL), e.pub_date DESC, e.id DESC"
...
def _download_one(
    ...
    try:
        written, _ = http_stream_to_file(enc, out_path)
        conn.execute(
...
def _parallel_map(func, rows: Iterable, jobs: int) -> List:
    if jobs <= 1:
    ...
    with ThreadPoolExecutor(max_workers=jobs) as ex:
        futs = [ex.submit(func, r) for r in rows]
    ...
    if jobs <= 1:
        ...
        def work(r):
            with connect_db() as c2:
                return int(_download_one(c2, feed, r, out_dir)[0])

        for res in _parallel_map(work, rows, jobs):
            ok += int(res)
...
def cmd_download_title(args):
    ...
    with connect_db() as conn:
        ...
        row = conn.execute(
            ...
            WHERE feed = ? AND LOWER(title) LIKE ?
    ...
    with connect_db() as conn:
        ...
        if args.fts:
            rows = list(
                ...
                WHERE e.feed = ? AND episodes_fts MATCH ?
            ...
            rows = list(
                conn.execute(
                    """
                    SELECT * FROM episodes
                    WHERE feed = ? AND (LOWER(title) LIKE ? OR LOWER(description) LIKE ?)
                    ORDER BY (pub_date IS NULL), pub_date DESC, id DESC
                    """,
...
def cmd_queue_download(args):
    ...
    def path_for_feed(feed: str) -> str:
        d = per_feed_dirs.get(feed)
        if not d:
            d = get_feed_download_dir(feed)
            os.makedirs(d, exist_ok=True)
            per_feed_dirs[feed] = d
        return d

    def work(r):
        feed = r["feed"]
        out_dir = path_for_feed(feed)
        with connect_db() as c2:
            success, _ = _download_one(c2, feed, r, out_dir)
            # delete by queue id (qid)
            c2.execute("DELETE FROM queue WHERE id = ?", (r["qid"],))
            c2.commit()
            return int(success)
...
def playlist_list(args):
    with connect_db() as conn:
        rows = list(
            conn.execute("SELECT name, created_at FROM playlists ORDER BY name ASC")
        )
    if not rows:
        print("No playlists.")
        return
...
def playlist_add(args):
    ...
    with connect_db() as conn:
        pl = conn.execute("SELECT 1 FROM playlists WHERE name = ?", (name,)).fetchone()
        if not pl:
            raise SystemExit(f"Playlist '{name}' not found. Create it first.")
        ep = conn.execute("SELECT id FROM episodes WHERE id = ?", (eid,)).fetchone()
        if not ep:
            raise SystemExit("Episode not found.")
...
def _unique_short_name(base: str, existing: Iterable[str]) -> str:
    base = slugify(base, maxlen=40) or "feed"
...
def cmd_opml_import(args):
    ...
    if not os.path.isfile(path):
        raise SystemExit(f"OPML file not found: {path}")
    try:
        root = ET.parse(path).getroot()
    except ET.ParseError as e:
        raise SystemExit(f"Failed to parse OPML: {e}")
    outlines = root.findall(".//outline")
    found = []
    for o in outlines:
        url = o.attrib.get("xmlUrl") or o.attrib.get("xmlurl") or ""
        text = o.attrib.get("title") or o.attrib.get("text") or url
        if url:
    ...
    if not found:
        ...
        return
    cfg = read_podfile()
    existing = set(cfg["feeds"].keys())
    added = 0
    with connect_db() as conn:
        now = now_iso()
        for title, url in found:
            short = _unique_short_name(title or url, existing)
            existing.add(short)
            cfg["feeds"][short] = url
            conn.execute(
                "INSERT OR REPLACE INTO feeds(short_name, url, custom_name, created_at, updated_at) VALUES (?, ?, ?, ?, ?)",
                (short, url, title, now, now),
            )
            added += 1
        write_podfile(cfg)
        conn.commit()
    print(f"Imported {added} feed(s) from {path}.")
...
def cmd_opml_export(args):
    ...
    feeds = cfg["feeds"]
    if not feeds:
        raise SystemExit("No feeds to export.")
    os.makedirs(os.path.dirname(os.path.abspath(path)) or ".", exist_ok=True)
    root = ET.Element("opml", version="2.0")
    head = ET.SubElement(root, "head")
    ET.SubElement(head, "title").text = f"{APP_NAME} Feeds"
    body = ET.SubElement(root, "body")
    for short, url in sorted(feeds.items()):
        summ = cfg["summaries"].get(short) or short
        ET.SubElement(body, "outline", type="rss", text=summ, title=summ, xmlUrl=url)
    tree = ET.ElementTree(root)
    tree.write(path, encoding="utf-8", xml_declaration=True)
    print(f"Exported {len(feeds)} feed(s) to {path}.")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    # list-feeds
    p = sub.add_parser(
        "list-feeds", help="List all configured podcast feeds (RSS only)"
    )
    ...
    p = sub.add_parser(
        "dirs",
        help="Show base download directory and per-feed directories"
    )
    ...
    # clean
    p = sub.add_parser("clean", help="Clear all download records for a podcast")
    p.add_argument("short_name", help="Feed short name")
    p.add_argument(
        "--delete-files",
        action="store_true",
    ...
    p.add_argument(
        "--all", action="store_true", help="Download all not-yet-downloaded episodes"
    )
    ...
    p.add_argument(
        "--jobs", type=int, default=1, help="Parallel downloads (default: 1)"
    )
    ...
    p.add_argument(
        "--fts", action="store_true", help="Use FTS5 full-text search if available"
    )
    ...
    # OPML import/export
    p = sub.add_parser("opml", help="Import/export feeds as OPML")
    opml_sub = p.add_subparsers(dest="opml_cmd", required=True)

    pi = opml_sub.add_parser("import", help="Import feeds from OPML file")
    pi.add_argument("path", help="Path to OPML file")
    pi.set_defaults(func=cmd_opml_import)

    pe = opml_sub.add_parser("export", help="Export feeds to OPML file")
    pe.add_argument("path", help="Path to write OPML file")
    pe.set_defaults(func=cmd_opml_export)
...
def cmd_set(args):
    ...
    if args.change_feed_dir:
        ...
        made_change = True
        print(
            f"Set custom dir for feed '{args.change_feed_dir}' → {os.path.abspath(os.path.expanduser(args.feed_dir_path))}"
        )
...
def main(argv=None):
    ...
    if not getattr(args, "cmd", None):
        ...
        sys.exit(1)
    try:
        # opml combined subparser dispatch
        if getattr(args, "opml_cmd", None):
            args.func(args)
        else:
            args.func(args)
